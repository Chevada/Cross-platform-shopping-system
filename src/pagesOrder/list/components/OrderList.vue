...
// 准备请求参数
const queryParmas: OrderListParams = {
  page: 1,
  pageSize: 5,
...
const getMemberOrderData = async () => {
  const result = await getMemberOrderAPI(queryParmas)
...
const onOrderPay = async (id: string) => {
  if (import.meta.env.DEV) {
    ...
    const result = await getPayWxPayMiniPayAPI({ orderId: id })
  ...
  <scroll-view scroll-y class="orders">
    <view class="card" v-for="item in orderList" :key="item.id">
      ...
      <view class="status">
        ...
        <!-- 订单状态文字 -->
        <text>{{ orderStateList[item.orderState] }}</text>
