...
const onOrderSubmit = async () => {
  if (!selectedAddress.value?.id) {
    return uni.showToast({ icon: 'none', title: '请设置收货地址' })
  }
  ...
  // 关闭当前页面并跳转
  uni.redirectTo({ url: `/pagesOrder/detail/detail?id=${result.result.id}` })
}
...
<template>
  <scroll-view scroll-y class="viewport" style="padding-bottom: 120rpx">
    <!-- 收货地址 -->
    <navigator
    ...
    <view class="goods">
      ...
      >
        ...
        <view class="meta">
          ...
          <view class="attrs">{{ item.attrsText.split(',').join(' ') }}</view>
          <view class="prices">
