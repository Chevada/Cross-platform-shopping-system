...
// 这个组件要被复用当作tabBar页面或者普通页面，这对普通页面，需要做底部安全区的适配
//获取屏幕的安全距离
const { safeAreaInsets } = uni.getSystemInfoSync()
console.log(safeAreaInsets)

// 获取调用本组件的页面传入的属性
const pageMode = defineProps<{
  mode: 'tabBar' | 'common'
}>()
...
const changeSelectedAll = () => {
  // 由于计算属性无法直接修改，我们可以对每一项的selected状态进行修改
...
<template>
  <scroll-view @scrolltolower="onScrolltolower" scroll-y class="scroll-view">
    ...
    <template v-if="memberStore.profile">
      ...
      <view
        class="toolbar"
        :style="{ paddingBottom: pageMode.mode === 'common' ? safeAreaInsets?.top + 'px' : 0 }"
      >
        <text @tap="changeSelectedAll" class="all" :class="{ checked: isSelectedAll }">全选</text>
    ...
    <view
      ...
      :style="{ height: (pageMode.mode==='common' ? 100 + (safeAreaInsets!.bottom) : 100) + 'px' }"
    ></view>
  </scroll-view>
</template>
